...
pub(crate) fn create_vertex2d_model(
    x: f32,
    y: f32,
    config: TextConfig,
    label: &str,
...
pub(crate) fn create_vertex2d_text(
    config: TextConfig,
    label: &str,
    pt: &Vertex2D,
) -> Option<RawImage> {
    Option::from(text_2d_image(config.clone(), || {
        String::from(format!("{}({:+08.2},{:+08.2})", label, pt.x, pt.y,))
    }))
}

pub(crate) fn create_vertex3d_model(
    x: f32,
    y: f32,
    label: &str,
    config: TextConfig,
...
pub(crate) fn create_vertex3d_text(
    config: TextConfig,
    label: &str,
    target: &Vertex3D,
) -> Option<RawImage> {
...
pub(crate) fn create_f32_text(config: TextConfig, label: &str, value: f32) -> Option<RawImage> {
    Option::from(text_2d_image(config.clone(), || {
        String::from(format!("{}{:+08.2}", label, value))
    }))
}

pub(crate) fn create_rect2d_model(
    x: f32,
    y: f32,
    config: TextConfig,
    label: &str,
...
pub(crate) fn create_rect2d_text(
    config: TextConfig,
    label: &str,
    rect: &Rectangle2D,
) -> Option<RawImage> {
