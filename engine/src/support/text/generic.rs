...
static ALPHABET: LazyLock<Mutex<HashMap<ColorSelection, HashMap<char, Vec<Vec<u8>>>>>> =
    LazyLock::new(|| Mutex::new(define_by_color()));
...
mod tests {
    ...
    use crate::support::text::generic::{create_generic, HEIGHT, WIDTH};
    ...
    fn one_letter() {
        ...
        assert_eq_color(
            Color::TRANSPARENT,
            Vec::from(image.data.drain(0..4).as_slice()),
        );
        assert_eq_color(
            Color::TRANSPARENT,
            Vec::from(image.data.drain(0..4).as_slice()),
